...
# stopwords = reset_stopwords()
cleaned = cleaner(data['full_desc'][0])
print(type(cleaned))

# corpus = to_corpus(data,'full_desc',stopwords)

# corpa = [list(tokenize(doc) for doc in corpus)]
# corpa = [ TaggedDocument(words, ['d{}'.format(idx)]) 
# for idx, words in enumerate(corpus)]

# model = Doc2Vec(corpa, vector_size=5, min_count=0)
